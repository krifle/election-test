...
먼저 구분해야 할 것은 `데이터 범위`와 `일치 기준`이다.

- 데이터 범위 `읍면동 총합`: 한 읍면동의 최종 개표 합계
- 데이터 범위 `관내사전투표`: 한 읍면동의 관내사전투표분만 따로 분리한 값
...
- 관내사전투표의 연도별 중앙값 `n`도 `2014년 557표 → 2018년 1,299표 → 2022년 1,532표`로 커졌고,
- 같은 기간 `top2` 완전 일치 쌍 수는 `44 → 14 → 2`로 줄었다.
